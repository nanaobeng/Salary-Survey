...
class QualForm(FlaskForm):
    ...
    workhours =IntegerField('A3. Number of working hours in a week:', validators=[DataRequired())
    ...
    marketpos = SelectField(u'B1. As an organisation, where (i.e. percentile terms) do you want to positionyourself on the market?', choices=[('twentyfive', '25'), ('forty', '40'),('fifty', '50'),
    ('sixty', '60'),('eighty', '80'),('ninety', '90')], validators=[DataRequired())
    salstruct = RadioField(u' B2(a). Does your organisation have a formal salary structure for staff ?', choices=[('yes', 'Yes'), ('no', 'No')], validators=[DataRequired())
    fileupload = FileField(u'B2(b). If Yes, please upload a copy of your current salary structure(s). Upload File', [validators.regexp(u'^[^/\\]\.pdf$')], validators=[DataRequired())
    salarylevel = TextAreaField('B2(c). If No, how are salary level(s) established?', validators=[DataRequired())
    salstructadjust = SelectField(u'B3. How often is salary structure(s) adjusted?', choices = [('annually','Annually'),('quarterly','Quarterly'),('bi-monthly','Bi-monthly'),('monthly','Monthly'),('other','Other')])
    specifyother = StringField('If other, please specify: ', validators=[DataRequired())
    adjdate = DateField('B4. Date of last adjustment:', validators=[DataRequired())
    ...
    howadjust = StringField('B8(b). If No, how are adjustments made?', validators=[DataRequired())
    evalsys = RadioField(u'B9(a). Does your organisation use a formal Job Evaluation System?',choices = [('yes','Yes'),('no','No')], validators=[DataRequired())
    altyes =  TextAreaField('B9(b). If Yes, what method of job evaluation is used?', validators=[DataRequired())
    periodicsal = SelectField(u'B10. What types of periodic salary increases are granted?',
    ...
    autocost = RadioField(u'B11(a). Do your staff receive automatic cost of living salary adjustments at periodic intervals?',choices = [('yes','Yes'),('no','No')], validators=[DataRequired())
    yesbasis = TextAreaField('B11(b). If Yes, on what basis are increments computed and what are the normalintervals?', validators=[DataRequired())
    gradenum = IntegerField('B12. How many grade levels do you have?', validators=[DataRequired())
    percdiff = StringField('B13. What is the percentage difference in salary between the notches within a grade?', validators=[DataRequired())
    gradebasis = RadioField(u'B14. Salary increments in the same grade are based on: ',choices = [('notches','Notches'),('percentages','Percentages')], validators=[DataRequired())
    staffprogress =  RadioField(u'B15. How do your staff progress through the steps? ',choices = [('performance/ Merits','Performance/ Merit'),('automatic annual salary increments','Automatic annual salary increments')])
    ...
    incentive = RadioField(u'C1(a). Does the organisation provide an annual cash bonus or other short term incentive plan (excluding 13th month pay) for employees?',choices = [('yes','Yes'),('no','No')])
    plans = TextAreaField('C1(b). Please list and describe these plans (provide copy of the plan or policy)')
    staffC1 = RadioField(u'C2(a). Are all staff including executives eligible for C1?',choices = [('yes','Yes'),('no','No')])
    explainno = TextAreaField('C2(b). If No, explain:')
    bonuses = RadioField(u'C3. When does your company pay the bonuses listed in 1b?',choices = [('option11','At the end of the year as a lump sum'),('option12','At completion of appraisal'),('option13','onthly as part of the salary'),('option14','Other (please sepcify)'))])
    month13bonus = RadioField(u'C4. Does your company pay 13th month bonus?',choices = [('yes','Yes'),('no','No')])
    bonustime = nuses = RadioField(u'C5. When is the 13th month bonus paid?',choices = [('option11','At the end of the year as a lump sum'),('option12','At completion of appraisal'),('option13','onthly as part of the salary'),('option14','Other (please sepcify)'))])
    ...
    reason= TextAreaField('D30(b). If Yes, please provide the reasons for a having a separate scale:', validators=[DataRequired())class QualForm(FlaskForm):
    name = StringField('Name', validators=[DataRequired())
    title = StringField('Title', validators=[DataRequired())
    address = StringField('Address', validators=[DataRequired())
    phone = IntegerField('Phone', validators=[DataRequired())
    ...
    turnover = TextAreaField('D29. Please indicate frequent reasons for employee turnover.:', validators=[DataRequired())
    scale = RadioField(u'D30. As an organisation, do you have a separate salary scale for local employees who have regional responsibilities within Africa?',choices = [('yes','Yes'),('no','No')], validators=[DataRequired())
    reason= TextAreaField('D30(b). If Yes, please provide the reasons for a having a separate scale:', validators=[DataRequired())v
