...
class QualForm(FlaskForm):
    ...
    work_hours = IntegerField('A2.How many working hours are there in a normal week?',validators=[DataRequired()])   
    employee_num_cat = IntegerField('A3. Fill in the number of employees for each category',validators=[DataRequired()])
    ...
    order = TextField('B17. In order of importance, what factors are considered in placing your new staff on your salary scale (please rank as 1st, 2nd, 3rd, etc)?', validators=[DataRequired()])
    ...
    max_salary = RadioField(u' B19. Is there a maximum salary for each grade which cannot be exceeded?',choices = [('yes','Yes'),('no','No')], validators=[DataRequired()])
